...
class MechanicPage extends StatefulWidget {
  ...
  MechanicPage({required this.arizaTuru,required this.arizaBolum});
...
class _MechanicPageState extends State<MechanicPage> {
  ...
  Future<void> _verileriGetir(String arizaTuru,String arizaBolum) async {
    print("arıza turu : ${arizaTuru}");
    try {
      final response = await http.post(Uri.parse("$baseUrl/ArizaGetirBolumeGore.php"), body: {
        'ARIZATURU': arizaTuru,
        'ARIZABOLUM': arizaBolum,
      });
...
// ZAMAN değerine göre sıralama
  ...
  void initState() {
    super.initState();
    _verileriGetir(widget.arizaTuru,widget.arizaBolum);
  }
